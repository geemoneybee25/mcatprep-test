...
const SkillDetail: React.FC<SkillDetailProps> = ({ 
  ...
  const getMasteryLabel = (p: number, status: SkillStatus) => {
    ...
    // Fixed: SkillStatus.LOCKED was used but it should be SkillStatus.NOT_READY
        ...
        <div className="lg:col-span-8 space-y-10">
          <section>
            ...
            <p className="text-slate-500 text-lg font-medium leading-relaxed max-w-2xl">
              Understanding mathematical relationships is foundational to SAT success. 
              In this unit, we explore how to manipulate complex expressions and interpret results in multiple contexts.
            </p>
          </section>
              ...
              <div className="absolute bottom-0 inset-x-0 p-8 bg-gradient-to-t from-black/80 to-transparent">
                 ...
                 <div className="flex justify-between items-center text-white/60 text-[10px] font-black uppercase tracking-widest">
                    <div className="flex gap-4">
                       <span className="text-white">Chapter 1: Foundations</span>
                       <span className="opacity-40">Chapter 2: Advanced Transformations</span>
                    </div>
                    <span>1080p HD</span>
                 </div>
              </div>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                 ...
                 <h3 className="font-bold text-slate-800 mb-2">Linear Modeling</h3>
                 <div className="text-sm text-slate-500 leading-relaxed mb-4">
                    <MathText text="$$y = mx + b$$" />
                 </div>
                 <ul className="text-xs space-y-2 text-slate-600 font-medium">
                   {/* Fix: Wrapped math symbols in MathText to ensure consistent rendering and avoid raw $ symbols in JSX text */}
                   <li>• <strong><MathText text="$m$" /></strong> represents the constant rate of change (slope)</li>
                   <li>• <strong><MathText text="$b$" /></strong> represents the initial value (y-intercept)</li>
                 </ul>
              </div>
              <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                 <div className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center font-bold mb-4">
                    {/* Fix: Use double backslash for LaTeX command \sum in JS string */}
                    <MathText text="$\\sum$" />
                 </div>
                 <h3 className="font-bold text-slate-800 mb-2">Equivalence</h3>
                 <div className="text-sm text-slate-500 leading-relaxed mb-4">
                    <MathText text="$$Ax + By = C$$" />
                 </div>
                 <ul className="text-xs space-y-2 text-slate-600 font-medium">
                   <li>• Isolate variables using inverse operations</li>
                   {/* Fix: Used MathText for the equation to avoid JSX expression parsing errors with curly braces and ensure correct KaTeX rendering */}
                   <li>• Multi-step equations require balanced transformations: <MathText text="$x = \\frac{C - By}{A}$" /></li>
                 </ul>
              </div>
            </div>
          </section>
